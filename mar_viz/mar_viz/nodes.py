...
class AvstackBridgedVisualizer(Node):
    def __init__(self):
        ...
        self.ego_publisher = self.create_publisher(MarkerArray, "ego", 10)

    def gt_init_pub_sub(self):
        # subscribe to ground truth object information
        self.gt_subscription = self.create_subscription(
            ObjectStateArray, "object_truth", self.gt_sub_callback, 10
        )
        ...
        for i in range(len(msg.boxes)):
            ...
            detections.markers.append(
                self.boundingBox3D_to_marker(
                    bounding_box_3D=msg.boxes[i], header=header, id=i, name_space="ego"
                )
            )

        return detections

    def boundingBox3D_to_marker(
        self,
            ...
            if self.gt_visualization_method == "arrow":
                points.markers.append(
                    self.objectState_to_marker_arrows(
                        object_state=msg.states[i], header=header, id=i, name_space="gt"
                    )
                )
    ...
    def objectState_to_marker_arrows(
        self, object_state: ObjectState, header: Header, id: int, name_space: str = "gt"
    ) -> Marker:
